...
export async function getDashboardStatsOptimized() {
  ...
  try {
    ...
    const result = await query(`
      ...
      attendance_stats AS (
        SELECT 
          ...
          ), 0) as today_attendance,
          COALESCE(COUNT(*) FILTER (
            WHERE check_in >= date_trunc('week', CURRENT_DATE)
          ), 0) as week_attendance
        FROM attendance
        WHERE 1=1 -- Fallback en caso de que la tabla no exista
      )
      SELECT 
        s.*,
        r.monthly_revenue,
        ...
        COALESCE(a.today_attendance, 0) as today_attendance,
        COALESCE(a.week_attendance, 0) as week_attendance,
      ...
      FROM stats s, revenue_stats r
      LEFT JOIN attendance_stats a ON true;
    `)
    ...
    const formattedStats = {
      ...
      pendingPaymentsCount: Number.parseInt(stats.pending_payments_count) || 0,
...
// Función optimizada para obtener estadísticas básicas (más rápida)
export async function getBasicStatsOptimized() {
  const cacheKey = "basic_stats"
  const cached = getCachedData(cacheKey)
  if (cached) return cached

  try {
    // Consulta más simple y rápida para estadísticas básicas
    const result = await query(`
      SELECT 
        COUNT(*) FILTER (WHERE status = 'active') as active_members,
        COUNT(*) FILTER (WHERE status = 'expired') as expired_members,
